# 基礎 10 · QC 的第二層：Doublet 與 Ambient RNA

**難度**：★★ ｜ **預估時間**：1 個工作天 ｜ **對應集數**：B5（⑤+ 策略室深化）、B9–B10
...
- 下載：`https://www.10xgenomics.com/datasets` 搜尋「10k PBMCs from a Healthy Donor (v3 chemistry)」，填 email 後**同時下載 filtered 與 raw 的 h5**（filtered/raw feature-barcode matrix）——raw 這次不是「B5 之前的世界」，它是估 ambient RNA 的原料。
...
5. 回答討論題：目標 10k、實收 11,769，超收對 doublet 率意味著什麼？如果這是你自己的實驗，上樣量你會怎麼決定？（B5 策略室：QC 問題有一半其實是實驗設計問題。）

### 階段 C：ambient RNA 與總清算（對應 B5 策略室、B9–B10）
...
- 這一層 QC 是所有進階題的隱藏前置：每張進階卡的資料都值得先問「doublet 清了嗎？ambient 估了嗎？」——尤其進階06/14/15 這類 per-sample 腫瘤資料（每個樣本要分開跑），以及任何你打算拿去發表的分析：審稿人現在會問這兩件事。
